...
void mainImage(out vec4 outputColor, vec2 coord) {
  ...
  transformedUv.y += sin(
    transformedUv.x * (uWarpFrequency * 1.5) + warpTime
  ) / (amplitude * 0.5);
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        delete container.dataset.webglFallback;
        ...
        const start = () => {
          if (!prefersReducedMotion && isPageVisible && animationId === 0) {
            animationId = window.requestAnimationFrame(loop);
          }
        };
